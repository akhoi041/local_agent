...
Previous completed pipeline: dev_notes/pipelines/TALOS_PIPELINE_030.md
Current active pipeline: dev_notes/pipelines/TALOS_PIPELINE_040.md
...
- Keep `dev_notes/pipelines/TALOS_PIPELINE_010.md`, `dev_notes/pipelines/TALOS_PIPELINE_020.md`, and `dev_notes/pipelines/TALOS_PIPELINE_030.md` as frozen history records.
- Track all 0.4.0 work in this file. During 0.4.0 development, project planning and helper checks should default to `dev_notes/pipelines/TALOS_PIPELINE_040.md`.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\pipeline_status.ps1 -Path dev_notes\pipelines\TALOS_PIPELINE_040.md
```
...
- [x] Preserve the completed 0.3.0 Beta pipeline as `dev_notes/pipelines/TALOS_PIPELINE_030.md`.
- [x] Create a dedicated 0.4.0 Pre-Alpha pipeline as `dev_notes/pipelines/TALOS_PIPELINE_040.md`.
- [x] Update `dev_notes/roadmap/TALOS_ROADMAP.md` so 0.3.0 is complete and 0.4.0 is active.
...
- [x] Run pipeline status for `dev_notes/pipelines/TALOS_PIPELINE_040.md`.
...
- `dev_notes/pipelines/TALOS_PIPELINE_040.md` status after Stage 0: Stage 0 is 5/5 and overall progress was 5/58 before this Stage 1 update.
...
- Dirty-state review: current 0.4.0 planning changes are `dev_notes/roadmap/TALOS_ROADMAP.md` and `dev_notes/pipelines/TALOS_PIPELINE_040.md`; `native/bin/talos_native.dll` is build output regenerated by the Stage 1 check and should be treated separately from source edits.
...
- [x] Confirm detection, verify, Codex polling, and history refresh do not spawn disruptive PowerShell/CMD windows during normal use.
- [x] Add or update timing thresholds for native detection, verify cache, support bundle generation, and installed-app health.
- [x] Add or update timing thresholds for diagnostics export and UI refresh paths introduced in 0.4.0.
- [x] Run folder/code cleanup to remove stale generated files, unused docs, dead code, and obsolete pipeline references.
- [x] Optimize repeated tool actions so Verify Sandbox, Save & Verify, support export, and diagnostics preview report progress clearly and do not block the main UI.
- [x] Audit the current Codex runtime dependency and document the 0.4.0 limitation clearly: Talos uses an available Codex runtime, may fall back to the VS Code extension runtime, and does not yet provide the 0.5.0 Runtime Manager.
- [x] Add regression tests for any product-readiness polish fixes.

Stage 8 implementation notes:

- Added `talos/performance.py` as the 0.4.0 performance guardrail source, with explicit thresholds for native detection, verify cache lookup, total/compile verify time, support bundle generation, diagnostics export, UI refresh, and installed-app health.
- Added `GET /api/performance_guardrails` and included the same guardrail payload in API support bundles so external testers can attach performance/runtime expectations without exposing source code or chat content.
- Confirmed normal-use subprocess paths keep Windows console windows hidden through `CREATE_NO_WINDOW`; developer/smoke helper scripts remain explicit tools rather than background app behavior.
- Cleaned stale generated cache and updated diagnostics documentation so 0.5.0 is now the Codex Runtime Manager step, not the older feedback-server direction.
- Recorded the 0.4.0 runtime limitation honestly: Talos can use an available Codex runtime and may fall back to the VS Code extension runtime, while first-class runtime discovery/pinning/sign-in readiness moves to 0.5.0.
- Added regression coverage for the performance guardrail schema, hidden-process policy, runtime-manager limitation, and state endpoint registration.
...
- [x] Run manual Arduino/Codex smoke matrix for simple `.ino`, multi-file `.h/.cpp`, AVR board, ESP32 board, conflict recovery, reconnect/cancel, support bundle, and troubleshooting docs.
- [x] Run diagnostics/privacy smoke: disabled-by-default, opt-in enable/disable, redacted export preview, and support-report attachment path.
- [x] Run UI/UX smoke across light, dark, neutral, normal window, maximized window, narrow window, and the main tool workflows.
- [x] Build final 0.4.0 Pre-Alpha release artifacts.
- [x] Sign artifacts or explicitly mark unsigned Pre-Alpha/Beta status.
- [x] Run installer smoke and installed-app smoke.
- [x] Run clean install/upgrade/uninstall validation and record evidence.
- [x] Generate final 0.4.0 distribution checklist with `-RequireReady`.
- [x] Bump app identity, release manifest naming, Inno fallback metadata, and release notes from 0.3.0 to 0.4.0 only at the final release gate.
- [x] Update release notes with 0.4.0 product-readiness fixes, UI/UX refresh, opt-in diagnostics behavior, known limitations, Codex runtime dependency notes, upgrade notes, and support instructions.

Stage 9 implementation notes:

- App identity, fallback identity, Inno metadata, release evidence defaults, Codex context package version, and release notes now target `0.4.0 Pre-Alpha` / `Talos-0.4.0-pre-alpha`.
- `scripts\pipeline_status.ps1` now defaults to `dev_notes\pipelines\TALOS_PIPELINE_040.md` so active helper checks no longer report the completed 0.3.0 pipeline by accident.
- Full automated checks passed through `python -B -m unittest tests.test_desktop_app`, `git diff --check` with only existing CRLF warnings, and `powershell -ExecutionPolicy Bypass -File scripts\check.ps1`.
- Release artifacts were generated in `releases\Talos-0.4.0-pre-alpha`: `Talos-0.4.0-pre-alpha.exe`, `Talos-0.4.0-pre-alpha-setup.exe`, `release_manifest.json`, and packaged release docs/config.
- Signing evidence is explicit and honest: `signing_status.json` records `unsigned-beta` with `documented_unsigned_prealpha_beta`; this release is not Authenticode signed.
- Installer, installed-app Arduino/Codex harness, and app-data lifecycle smoke evidence passed through `installer_smoke.json`, `installed_app_smoke.json`, and `app_lifecycle_smoke.json`.
- `DISTRIBUTION_CHECKLIST.md` was generated with `-RequireReady` and passed the required script gates for artifacts, signing/unsigned status, installer smoke, installed-app smoke, and app-data lifecycle.
- The installed-app smoke includes an automated Arduino/Codex harness for edit, save, verify, staged patch, apply, save-to-workspace, and verify-again. The checklist still records broader external tester approval separately, so public handoff should review the distribution decision boxes before publishing.
- Diagnostics/privacy and UI/UX readiness are covered by the 0.4.0 regression tests, Stage 3/4/6 notes, and the final release documentation; hosted telemetry remains deferred and disabled by design.
- Build validation used `-AllowDirty` because the release-gate edits and evidence were still in the working tree. For a public GitHub release, rebuild once from the committed/tagged tree without `-AllowDirty` to get the cleanest provenance.
...
- Architecture Slimming: docs taxonomy enforcement, frontend module split, Python boundary cleanup, native-helper benchmark planning, and size/performance guardrails before adding new target apps.
