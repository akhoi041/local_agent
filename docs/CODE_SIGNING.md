...
Talos 0.8.0 Beta may be distributed unsigned while the publisher certificate is being prepared, but that state must be explicit in the release folder. Public commercial releases should be signed before distribution.
...
For 0.8.0, the current release posture is explicit unsigned Beta unless a T-Engine code-signing certificate is configured before the release gate.
