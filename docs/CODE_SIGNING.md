...
Talos 0.7.5 Beta may be distributed unsigned while the publisher certificate is being prepared, but that state must be explicit in the release folder. Public commercial releases should be signed before distribution.
...
For 0.7.5, the current release posture is explicit unsigned Beta unless a T-Engine code-signing certificate is configured before the release gate.
